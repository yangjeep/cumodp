...
printf "#include \"../include/power_inversion.h\"\n" >> $TEMP_TEST_SRC
...
printf "sfixn m = atoi (argv[3]);\n" >> $TEMP_TEST_SRC
printf "sfixn l = atoi (argv[4]);\n" >> $TEMP_TEST_SRC
printf $TEST"(p,k,m,l);\n" >> $TEMP_TEST_SRC
printf "return 0; }\n" >> $TEMP_TEST_SRC
...
cp ../power_series_inversion.input ./
...
for k in 3 5 8 10 12 $((2**8)) $((2**9)) $((2**10));
do
	for m in 2 4 8;
	do
		for l in 2 $k;
		do
			./$TEST $p $k $m $l > $TMP
			cat $TMP | grep -e "\[" > $MW
			echo "read(\"power_series_inversion.input\");" >> $MW
			echo "err:=0;" >> $MW
			echo "for jj from 0 to $(($m-1)) do" >> $MW
			echo "v := verify_inverse_mod(F[jj],G[jj],x,$l,$p);" >> $MW
			echo "if v <> 1 then err := 1; end if;" >> $MW
			echo "od; " >> $MW
			...
			echo "k=$k, m=$m, l=$l : $RESULT" >> ../$LOG
			echo "k=$k, m=$m, l=$l : $RESULT" 
		done
	done
done
