...
static void M_DECL NopFunction() { /* do nothing */ }
...
///////////////////////////////////////////////////////////////////////////////
// Functions to convert a coefficient vector to a maple DAG
// Written by Roman Pearce, adjusted by WP
///////////////////////////////////////////////////////////////////////////////

/* Maple stores machine sized integers inplace */
/* by shifting left and setting the lowest bit */
/* it is called the "immediate integer" format */
/* |a| < 2^62 or 2^30 */

/* This is 32-bit version */
#define IMMEDIATE(x) (((sfixn)(x) << 1) | 1)
#define VALUE_IMMEDIATE(x) ((sfixn)(x) >> 1)

/* Machine dependent version */
/* #define IMMEDIATE(x) (((M_INT)(x) << 1) | 1) */
/* #define VALUE_IMMEDIATE(x) ((M_INT)(x) >> 1) */

/**
 * Recursively build a maple DAG from its coefficient vector.
 * @A, coefficient vector 
 * @B, bound vector (first slot is used.)
 * @v, list of variables,
 * @m, main variable index, starting from 1
 */
ALGEB rdr2dag_inner(MKernelVector kv, sfixn *A, sfixn *B, ALGEB v, sfixn m) 
{
    sfixn i, sz = 1;
    ALGEB f, t, s;

    if (m == 1) {
	    /* for n terms, the sum object is 2*n+1 words */
	    f = kv->allocALGEB(2*B[m-1]+1, MAPLE_SUM);
	    for (i=0; i < B[m-1]; i++) {
		    /* for m variables, the prod is 2*m+1 */
		    /* for bottom univariate level it's 3 */
		    /* in recursive representation it's 5 */
		    t = kv->allocALGEB(3, MAPLE_PROD);
		    t[1] = v[1];		           /* variable */
		    t[2] = (M_INT **)IMMEDIATE(i); /* exponent */
		    f[1+2*i] = (M_INT **)t;		   /* monomial */
		    f[2+2*i] = (M_INT **)IMMEDIATE(A[i]); /* coefficient */
	   }
       return f;
    } 

    f = kv->allocALGEB(2*B[m-1]+1, MAPLE_SUM);
    /* the coefficient size is sz */
    for (i = 0; i < m-1; ++i) { sz *= B[i]; }

    for (i=0; i < B[m-1]; ++i) {
        /* coefficient from the recursive call */
        s = rdr2dag_inner(kv, A + i * sz, B, v, m - 1);
        /* in recursive representation it's 5 */
        t = kv->allocALGEB(5, MAPLE_PROD);
        t[1] = (M_INT **)s;                /* coefficient in v[m]^i */
        t[2] = (M_INT **)IMMEDIATE(1);       
        t[3] = (M_INT **)v[m];             /* v[m]^i */
        t[4] = (M_INT **)IMMEDIATE(i);

        f[1+2*i] = (M_INT **)t;		       /* monomial */
        f[2+2*i] = (M_INT **)IMMEDIATE(1); /* coefficient */
   }
   return f;
}

/**
 * Build a maple DAG from its coefficient vector.
 *
 * conv := define_external(rdr2dag, 'MAPLE', 'LIB'="conv32.so"):
 * f := conv(rtable([1,2,3,4,5,6], datatype=integer[4]), rtable([3,2]),[x,y]);
 *
 * 1 + 2*x + 3*x^2 + (4 + 5*x + 6*x^2)*y
 *
 */
#ifdef WINDOWS
__declspec(dllexport) ALGEB __stdcall
...
ALGEB
#endif
rdr2dag(MKernelVector kv, ALGEB *args)
{
	sfixn *A, *B;
	ALGEB f;
	ALGEB a = args[1];	/* rtable of coefficients */
	ALGEB b = args[2];	/* rbable of bounds */
	ALGEB v;	        /* list of variables */
    sfixn m = VALUE_IMMEDIATE(args[4]);  /* number of variables */

	/* a list in Maple is a pointer to a sequence */
	/* Maple sequence are arrays of machine words */
	/* we want the sequences for b and for v */
	v = (ALGEB)(args[3][1]);

	/* note that we did not do any error checking */
	/* that requires the macros for ID and LENGTH */

	/* get the rtable data block (C array) */
	A = kv->rtableData(a);
	B = kv->rtableData(b);

	/* Maple's sum format: (product, coefficient) */
	/* coefficient is an integer, so in recursive */
	/* representations the recursive part goes in */
	/* the product and the coefficient would be 1 */
	/* Maple's prod format: (an object, exponent) */

    f = rdr2dag_inner(kv, A, B, v, m);
	f = kv->simplify(f);
	return f;
}
...
/**
 * Convert a triangular decomposition to a list of polynomials:
 *
 * @kv : Maple kernel pointer
 * @dec : linked-queue of regular_chain2
 * @v : list of variables
 * 
 * Return: a sequence of polynomials 
 * 
 * For example, let dec = [[A1, B1], [NULL, B2], [A3, NULL]]
 *
 * Then the output is a sequence of length 6
 *
 * A1, B1, 0, B2, A3, 0
 * 
 * which will be converted into a sequence of regular chains 
 * in the high level code.
 *
 **/
static ALGEB decomp_to_maple_dag(MKernelVector kv, LinkedQueue *dec, ALGEB v) {
    ALGEB seq, a, b;
    LinearNode *current;
    preFFTRep *A, *B;
    sfixn i, n = dec->count;
    sfixn bounds[2] = {0, 0}; // partial size vector of a uni(bi)-variate poly
    
    if (n <= 0) return ToMapleNULL(kv);

    /* The list has length n, each pair has two polynomials  */
    seq = kv->allocALGEB(2 * n + 1, MAPLE_LIST);
    for (i = 1, current = dec->front; 
                current != NULL; 
                current = current->next) 
    {
        /* Convert A into a Maple univariate polynomial */
        A = ((regular_chain2 *)(current->element))->poly0;
        if (A == NULL) { 
            a = (ALGEB)IMMEDIATE(0); 
        } else {
            bounds[0] = BUSZSI(A, 1) + 1; 
            a = rdr2dag_inner(kv, DAT(A), bounds, v, 1);
        }
        seq[2*i-1] = (M_INT**)a;

        /* Convert B into a Maple bivariate polynomial */
        B = ((regular_chain2 *)(current->element))->poly1;
        if (B == NULL) {
            b = (ALGEB)IMMEDIATE(0);
        } else {
            bounds[0] = BUSZSI(B, 1) + 1; 
            bounds[1] = BUSZSI(B, 2) + 1; 
            b = rdr2dag_inner(kv, DAT(B), bounds, v, 2);
        }
        seq[2*i] = (M_INT**)b;
 ...
 *
 *     Output:
 *       MaplePointer of a LinkedQueue
 *
 * - "free"
 *     Free the decomposition in C
 *
 *     Input:
 *       args[1]: option
 *       args[2]: LinkedQueue *val; // decomposition
 *     Output:
 *       Maple NULL pointer
 *
 * - "print"
 ...
 *
 * - "build_dag"
 *     Build the maple DAG directly in C
 *    
 *     Input:
 *       args[1]: option
 *       args[2]: LinkedQuene *val; // decomposition
 *       args[3]: variable list
 *     Output:
 *       A maple sequence of polynomials or 
 *       NULL if the decomposition is an empty linked list
 * 
 *  WARNING, the following code is deprecated.
 *
 * - "num_of_components" 
 *     Return the number of components in the output
 *     Input:
 *       args[1]: option
 *       args[2]: LinkedQueue *val; // decomposition
 *     Output:
 ...
 *
 * - "deg_info"
 *     Obtain the size information of the output
 *     Input:
 *       args[1]: option
 *       args[2]: LinkedQueue *val; // decomposition
 *       args[3]: sfixn *pdgs;      // sizes of each components
 *     Output:
 *       Fill the size info into dgs;
 *
 *   For example, when
 *
 *     dec = [(A1, B1), (A2, NULL), (NULL, B3), (A4, B4)]
 *
 *   then the array is of length 12 = 3 * 4
 *   pdgs = {
 *      deg(A1, x), deg(B1, x), deg(B1, y);
 *      deg(A2, x),         -1,         -1;
 *              -1, deg(B3, x), deg(B3, y);
 *      deg(A4, x), deg(B4, x), deg(B4, y); }
 *
 * - "convert"
 *     Convert the triangular decomposition into a maple object
 *
 *     Input:
 *       args[1]: option
 *       args[2]: LinkedQueue *val; // decomposition
 *       args[3]: sfixn *coeff;     // coefficients of each components
 *     Output:
 *       Fill the coefficients into coeff;
 */
...
bivariate_solve_ALGEB(MKernelVector kv, ALGEB *args)
{
    ...
    sfixn p;                     /* fourier prime */
    sfixn *pdgs;                 /* partial degree vector of the decompsition */
    sfixn *coeff;                /* coefficient vector of the decomposition */
    LinearNode *current;         /* iterate through the queue */
    preFFTRep *A, *B;            /* (A, B) is regular chain */
    ALGEB v;                     /* variable sequence in a rtable */
    ...
    if (strcmp(option, "solve" ) == 0) {
        ...
        queue = EX_ModularSolve2(F1, F2, p);
        
        freeOneWrapperPoly(F1);
        freeOneWrapperPoly(F2);
        if (queue == NULL) {
            MapleRaiseError1(kv, "fail to solve the input system %1", args[1]);
            return ToMapleNULLPointer(kv);
        } else {
            KV = kv;
            val = ToMaplePointer(kv, (void *)queue, (M_INT)&MarkLinkedQueue);
            MaplePointerSetMarkFunction(kv, val, MarkLinkedQueue);
            MaplePointerSetDisposeFunction(kv, val, DisposeLinkedQueue_RChain2);
            return val;
        }
    } else if (strcmp(option, "free") == 0 ) {
        /* free the Gc decomposition */
        if (DEBUG) {
            if(!IsMaplePointer(kv, args[2]) ||
                MaplePointerType(kv, args[2]) != (M_INT)&MarkLinkedQueue)
            {
                MapleRaiseError(kv, "gc LinkedQueue expected");
                return ToMapleNULLPointer(kv);
            }
        }
        queue = (LinkedQueue *)MapleToPointer(kv, args[2]);
        if (queue != NULL) {
            EX_LinkedQueue_Free(queue, EX_RegularChain2_Free);
            MaplePointerSetDisposeFunction(kv, args[2], NopFunction);
        }
        ...
        queue = (LinkedQueue *)MapleToPointer(kv, args[2]);
        EX_LinkedQueue_Print(queue, EX_RegularChain2_Print);
        return ToMapleNULL(kv);
    } else if (strcmp(option, "build_dag") == 0) {
        if (DEBUG) {
            if(!IsMaplePointer(kv, args[2]) ||
               MaplePointerType(kv, args[2]) != (M_INT)&MarkLinkedQueue)
            {
	    ...
	    v = (ALGEB)(args[3][1]); /* list of variables */
        queue = (LinkedQueue *)MapleToPointer(kv, args[2]);
        return decomp_to_maple_dag(kv, queue, v);
    } else if (strcmp(option, "num_of_components") == 0) {
        if (DEBUG) {
            if(!IsMaplePointer(kv, args[2]) ||
               MaplePointerType(kv, args[2]) != (M_INT)&MarkLinkedQueue)
            {
        ...
        queue = (LinkedQueue *)MapleToPointer(kv, args[2]);
        num = EX_LinkedQueue_Size(queue);
        return ToMapleInteger(kv, num);
    } else if (strcmp(option, "deg_info") == 0) {
        /* get the degree info of the decomposition */
        if (DEBUG) {
            if(!IsMaplePointer(kv, args[2]) ||
                MaplePointerType(kv, args[2]) != (M_INT)&MarkLinkedQueue)
            {
        ...
        queue = (LinkedQueue *)MapleToPointer(kv, args[2]);
        pdgs = (sfixn *)RTableDataBlock(kv, args[3]);
        // Iterated through the queue, should be replaced
        // by generic implementations
        current = queue->front;
        for (i = 0; current != NULL; current = current->next, ++i) {
            ...
            if (A != NULL) 
                ...
                pdgs[3 * i] = -1;
            if (B != NULL) {
                pdgs[3 * i + 1] = BUSZSI(B, 1);
                pdgs[3 * i + 2] = BUSZSI(B, 2);
            } else
                pdgs[3 * i + 1] = pdgs[3 * i + 2] = -1;
        }
        return ToMapleNULL(kv);
    } else if (strcmp(option, "convert") == 0) {
        if (DEBUG) {
            if(!IsMaplePointer(kv, args[2]) ||
               MaplePointerType(kv, args[2]) != (M_INT)&MarkLinkedQueue)
            {
        ...
        queue = (LinkedQueue *)MapleToPointer(kv, args[2]);
        coeff = (sfixn *)RTableDataBlock(kv, args[3]);
        for (current = queue->front; current != NULL; current = current->next) {
            ...
            //printf("SIZ(B) = %d\n", SIZ(B));
            if (A != NULL) { 
                memcpy(coeff, DAT(A), sizeof(sfixn)*SIZ(A));
                coeff += SIZ(A);
            }
            if (B != NULL) {
                memcpy(coeff, DAT(B), sizeof(sfixn)*SIZ(B));
                coeff += SIZ(B);
            }
        }
        return ToMapleNULL(kv);
