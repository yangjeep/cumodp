...
#if defined(_cuda_modp_)
    // GPU code
    if (CUDA_TAG && e1 >= 14 && can_call_fftmul_uni(degA, degB)) {
        if (DEBUG) printf("Calling GPU code with size 2^%d\n", e1);
        if (DEBUG) printf("degA = %d, degB = %d\n", degA, degB);
        ...
        if (err == CUMODP_SUCCESS) {
            if (DEBUG) printf("cuda code succeeds and returns\n");
            return; 
        } else {
