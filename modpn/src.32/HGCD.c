...
static 
void neg(sfixn* C, sfixn dC, sfixn PPP){
  sfixn i;
  for (i = 0; i <= dC; i++)
...
static 
void copy(sfixn* C, sfixn *dC, sfixn *A, sfixn dA){
  sfixn i;
  for (i = 0; i <= dA; i++)
    C[i] = A[i];
  dC[0] = dA;
...
static 
void mul(sfixn* C, sfixn *dC, sfixn* A, sfixn dA, sfixn* B, sfixn dB,MONTP_OPT2_AS_GENE * pPtr){
  sfixn e,n;
  if (dA == -1 || dB == -1){
    dC[0] = -1;
    return;
...
void HGCD(matrix *T, sfixn *A, sfixn dA, sfixn *B, sfixn dB, MONTP_OPT2_AS_GENE * pPtr){
  ...
  if (dB >= dA){
    printf("HGCD: error in the degrees.\n");
    exit(-1);
  } // we can assume dA > dB
  ...
  // Euclidean division
  // tmp2 = Q
  quo(tmp2, &dTmp2, tA, dTa, tB, dTb, pPtr->P);
...
void XGCD(sfixn *C, sfixn *dC, sfixn *D, sfixn *dD, sfixn *G, sfixn *dG, 
  ...
  // tmp2 = Q
  quo(tmp2, &dTmp2, A, dA, B, dB, pPtr->P);
  neg(tmp2, dTmp2, pPtr->P);
  // tmp = A
  copy(tmp, &dTmp, A, dA);
  // A = B
